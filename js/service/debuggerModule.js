...
( function( pacManApp ){
    ...
    pacManApp.service.debuggerModule = function( options ) {        
        // PRIVATE:
        ...
        // FUNCTION: extendOptions( void ) void
            ...
            // set on
            self.on = self.options['on'] !== undefined && self.options['on'] === true ? true : false; 
            
            // loop over layout options
            for( let index of Object.keys( self.layoutOptions ) ){
            
                // over ride option exists
                if( self.options['layoutOptions'][index] !== undefined ){
            
                    // set value
                    self.layoutOptions[index] = self.options['layoutOptions'][index];
